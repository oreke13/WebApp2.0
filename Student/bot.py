...
async def start_command(message: types.Message):
    ...
    try:
        response = requests.post('https://d486-185-250-30-189.ngrok-free.app/register/',
                                 data={'telegram_id': telegram_id, 'username': username})

        # Проверяем успешность регистрации
        if response.status_code == 200 and response.json().get('status') == 'success':
            # Передаем telegram_id через URL для мини-приложения
            web_app_url = f"https://d486-185-250-30-189.ngrok-free.app/?telegram_id={telegram_id}"  # URL мини-приложения с параметром
